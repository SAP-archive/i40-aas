...
const { STORAGE_ADAPTER_MONGODB_MONGODB_PORT = 3100 } = process.env;
//const server = http.createServer(router);

router.listen(STORAGE_ADAPTER_MONGODB_MONGODB_PORT, () => {
  console.log(
    `A Server is running http://localhost:${STORAGE_ADAPTER_MONGODB_MONGODB_PORT}...`
  );
